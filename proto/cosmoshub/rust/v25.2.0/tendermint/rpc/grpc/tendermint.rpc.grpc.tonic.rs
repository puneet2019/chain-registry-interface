...
*/
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        > {
            ...
            req.extensions_mut()
                .insert(
                    GrpcMethod::new("tendermint.rpc.grpc.BroadcastAPI", "BroadcastTx"),
                );
            self.inner.unary(req, path, codec).await
        }
    }
...
pub mod broadcast_api_server {
    ...
    #[async_trait]
    pub trait BroadcastApi: std::marker::Send + std::marker::Sync + 'static {
        ///
        async fn ping(
            &self,
...
*/
    #[derive(Debug)]
    pub struct BroadcastApiServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for BroadcastApiServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "tendermint.rpc.grpc.BroadcastAPI";
    impl<T> tonic::server::NamedService for BroadcastApiServer<T> {
